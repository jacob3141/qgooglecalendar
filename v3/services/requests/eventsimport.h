...
class EventsImport : public RequestOperation {
public:
    EventsImport(RequestOperationDelegate *requestDelegate, QObject *parent = 0)
        : RequestOperation(requestDelegate, parent) {
    }

    void setParameters(QString calendarId, Event event) {
    ...
    QNetworkRequest networkRequest() {
        QNetworkRequest networkRequest;
        networkRequest.setUrl(QString("%1/calendars/%2/events/import")
                              .arg(baseUrl())
                              .arg(_calendarId));

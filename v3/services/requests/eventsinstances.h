...
class EventsInstances : public RequestOperation {
public:
    EventsInstances(RequestOperationDelegate *requestDelegate, QObject *parent = 0)
        : RequestOperation(requestDelegate, parent) {
    }

    void setParameters(QString calendarId, QString eventId) {
    ...
    QNetworkRequest networkRequest() {
        QNetworkRequest networkRequest;
        networkRequest.setUrl(QString("%1/calendars/%2/events/%3/instances")
                              .arg(baseUrl())
                              .arg(_calendarId)
                              .arg(_eventId));

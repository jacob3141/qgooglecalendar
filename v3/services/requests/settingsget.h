...
class SettingsGet : public RequestOperation {
public:
    SettingsGet(RequestOperationDelegate *requestDelegate, QObject *parent = 0)
        : RequestOperation(requestDelegate, parent) {
    }

    void setParameters(QString setting) {
        _setting = setting;
    }

    QNetworkRequest networkRequest() {
        QNetworkRequest networkRequest;
        networkRequest.setUrl(QString("%1/users/me/settings/%2")
                              .arg(baseUrl())
                              .arg(_setting));

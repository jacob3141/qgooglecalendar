...
class AclWatch : public RequestOperation {
public:
    AclWatch(RequestOperationDelegate *requestDelegate, QObject *parent = 0)
        : RequestOperation(requestDelegate, parent) {
    }

    void setParameters(Calendar calendar, int ruleId) {
        _calendar = calendar;
        _ruleId = ruleId;
    }
